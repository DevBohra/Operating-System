...
{   
    ...
    memcpy(buffer, m[address], 41); // copy memory contents to buffer
    outFile << buffer << endl;      // print buffer contents to file, end with newline
...
void cpu ::terminate(vector<int> em)
{
    ...
    if(em[0] == 0) {
	cout << " \n Normal Termination" << endl; // normal termination if error message is zero
	hRead = true;
    } else {
	cout << "\n Abnormal Termination" << endl; // abnormal termination otherwise
	for(int i = 0; i < em.size(); i++) {
	    cout << errorMessage[em[i]] << endl; // print out all error messages
	}
	error = true;
	hRead = true;
    }
